...
public class SceneObject : MonoBehaviour {
    ...
    public Collider2D m_sceneBoundingBox;
    ...
    public bool GoThroughEntrance (SceneEntrance type) {
        ...
        if (targetEntrance != null) {
            if (targetEntrance.active) {
                ...
                Debug.Log ("Attempted to go to the entrance " + targetEntrance.type + " but failed, as the entrance is inactive");
                return false;
            }
        } else {
            Debug.Log ("Attempted to go to the entrance " + type + " but failed, as no target entrance could be found.");
            return false;
        }
    }
            ...
            foreach (Entrance entrance in entrances) {
                ...
                if (entrance.connectsTo != null && entrance.active) {
                    ...
                    foreach (Entrance targetEntrance in entrance.connectsTo.entrances) {
                        if (targetEntrance.type == entrance.connectEntrance) {
    ...
    void MakeTwoWayConnections () {
        // Makes all doors work in both directions
        if (entrances.Count > 0) {
            foreach (Entrance entrance in entrances) {
                if (entrance.connectsTo != null && entrance.active) {
                    foreach (Entrance targetEntrance in entrance.connectsTo.entrances) {
                        if (targetEntrance.type == entrance.connectEntrance) {
                            targetEntrance.connectsTo = this;
                            targetEntrance.connectEntrance = entrance.type;
                        }
                        /*
