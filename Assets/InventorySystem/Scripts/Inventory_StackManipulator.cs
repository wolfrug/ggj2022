...
namespace Inventory {
    ...
    public class Inventory_StackManipulator : MonoBehaviour {
        ...
        public void StartManipulator (Item_DragAndDrop targetBox, Item_DragAndDrop othercombinerItem, bool autoCombine = false) {
            ...
            if (pool != null) {
                pool.SetMinMax (new Vector2 (0, slider.maxValue));
                if (startAtMax) {
                    pool.Value = slider.maxValue;
                } else {
                    pool.Value = 0f;
                }
                pool.ChangePool (0f);
