...
namespace Inventory {
 ...
 UI = 3000,
    ...
    public class GenericContextMenu : MonoBehaviour { // Add to regular Unity dropdown menu. Slightly hacky. But works! Mix with clicker of some kind.
        ...
        public static GenericContextMenu GetWorldMenuWithDirection () {
            // Attempts to get either a left or right facing context menu depending on where the cursor is, if not return whatever it can find
            ContextMenuType type = ContextMenuType.WORLD_LEFT;
            if (Input.mousePosition.x > Screen.width / 2.0f) {
                type = ContextMenuType.WORLD_RIGHT;
            }
            foreach (GenericContextMenu menu in allContextMenus) {
                if (menu.type == type) {
                    return menu;
            ...
            Debug.LogWarning ("Could not find World Direction Context Menu, attempting to return regular World");
            return GetMenuOfType (ContextMenuType.WORLD);
        }
