        ...
        public void SelectItem (UI_ItemBox selectedItem) {
            ...
            if (selectedItem.draggable) { // Can be dragged, therefore can be moved around
                ...
                if (targetInventory != null) {
                    if (pickUpToPlayerInventory && (targetInventory.type == InventoryType.PLAYER) && selectedItem.data.IsPermittedInventory (targetInventory.type)) {
                        addedEntries.Add (ContextMenuEntryType.UI_TAKE);
                    } else if (selectedItem.data.IsPermittedInventory (targetInventory.type)) {
