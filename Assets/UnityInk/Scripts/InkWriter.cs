...
public class InkWriter : MonoBehaviour {
    ...
    public static bool storyLoaded = false;
    ...
    public string linebreakCharacters = "\n\n";
    ...
    public void StartStory () {
        ...
        if (!SaveManager.instance.IsNewGame) {
            if (ES3.KeyExists (SaveManager.instance.CurrentSlot + inkStoryObject.storyName + "savedInkStory")) {
                savedJson = ES3.Load<string> (SaveManager.instance.CurrentSlot + inkStoryObject.storyName + "savedInkStory");
            };
            if (savedJson != "") {
            ...
            ES3.DeleteKey (SaveManager.instance.CurrentSlot + inkStoryObject.storyName + "savedInkStory");
            if (mainWriter) { GoToKnot ("start"); };
        }
        InkWriter.main.story.variablesState["debug"] = false;
        RefreshView ();
        if (mainWriter) { storyLoaded = true; }; // Static variable that can be referenced to see if things have loaded
    }
    ...
    public void SaveStory () {
        ...
        ES3.Save<string> (SaveManager.instance.CurrentSlot + inkStoryObject.storyName + "savedInkStory", InkWriter.main.story.state.ToJson (), SaveManager.instance.settings);
        SaveManager.instance.AddSlotKey (SaveManager.instance.CurrentSlot + inkStoryObject.storyName + "savedInkStory");
